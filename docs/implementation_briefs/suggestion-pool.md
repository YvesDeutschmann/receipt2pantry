...
The most important intelligence in this feature is depletion-aware planning: when generating suggestions for more than one meal slot in a single run, the algorithm doesn't just look at the pantry as it exists right now. It simulates what the pantry will look like after earlier slots in that run are hypothetically cooked, so that later suggestions are realistic given what ingredients will still be available. (The planning window is one day, not a week.) This simulation is purely for planning purposes — the real pantry is only decremented when a user marks a meal as actually cooked, which is existing behavior that does not change.
